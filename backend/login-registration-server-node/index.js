...
app.get("/getAllLots", async (req, res) => {
  try {
    const { search, center, page = 1, limit = 10 } = req.query; // Destructure query params
    const filter = {};

    // Search by Lot No
    if (search) {
      filter.lotNo = { $regex: search, $options: "i" }; // Case-insensitive search by LotNo
    }

    // Search by Center
    if (center) {
      filter.center = { $regex: center, $options: "i" }; // Case-insensitive search by Center
    }

    // Pagination settings
    const skip = (page - 1) * limit;
    const lots = await Lot.find(filter).skip(skip).limit(parseInt(limit));
    const total = await Lot.countDocuments(filter); // Get total count based on filter

    res.json({ data: lots, total });
  } catch (error) {
